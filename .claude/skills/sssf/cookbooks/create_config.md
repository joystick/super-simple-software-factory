...
```yaml
defaults:
  coding_agent: claude_code        # claude_code (headless `claude -p`) | pi
  model: anthropic/claude-sonnet-4-6   # ALWAYS provider/model-id — a bare id is ambiguous
  thinking: medium                 # off | minimal | low | medium | high | xhigh | max
  harness_engineering: []          # pi extension names; IGNORED under claude_code

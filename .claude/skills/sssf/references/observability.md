...
**Spend is itemised per phase.** `agent_end.usage` carries tokens *and* dollars for each component pi reports — `input`, `output`, `cache_read`, `cache_write` — summed across every send the phase made, so a phase that retried on a bad envelope or a failed gate shows what all its attempts cost, not just the last one. `total_tokens` is the CLI's own reported total when it reports one (pi's `totalTokens`), else the sum of the four components — empirically the two agree, but the field is defined as reported-else-summed, not as the sum (see `UsageBreakdown.add_turn()`); the component costs sum to `total_cost`; the visualizer's Cost panel renders them as a table you can add up by eye.
